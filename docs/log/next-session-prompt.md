...
- Static site + one PHP handler, built by `node engine/build-site.mjs --site=comida`. 266 HTML pages / 251 in the sitemap: 150 recipes, 24 guides, 32 meat cuts, 5 catering landing pages, 12 `/recetas-con/` ingredient pages, 5 seasonal collections, catering pages, `/mercado/` (canasta, queso Paraguay, carne a consultar), `/planificador/`, printable recetarios, viandas (noindex behind a partner gate).
- Plan 13 (meat/fish) is BUILT; next: photos for the new recipes, then the backlog below.
