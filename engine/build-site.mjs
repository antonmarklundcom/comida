...
function header(){
 ...
 html=html.replace(/(<div class="drawer">)[\s\S]*?(<\/div>\s*<\/details>)/,`$1<p class="eyebrow">Catering</p>${nav.map(r=>`<a href="${attr(r.path)}">${esc(r.label)}</a>`).join('')}<div class="main"><p class="eyebrow">Cocina</p><a href="/recetas/">Recetas</a><a href="/carne/">Carne</a><a href="/guias/">Guías</a>${routes.some(r=>r.id==='guia-mercado-de-abasto'&&r.published)?'<a href="/mercado-de-abasto/">Mercado de Abasto</a>':''}<p class="eyebrow">Catering</p><a href="/como-funciona/">Cómo funciona</a><a href="/precios/">Precios</a><a href="/proveedores/">Sumá tu servicio</a><a href="/contacto/">Contacto</a></div>$2`);
...
const analytics=config.analytics?.enabled&&/^G-[A-Z0-9]+$/.test(config.analytics.publicId||'')?`<script async src="https://www.googletagmanager.com/gtag/js?id=${config.analytics.publicId}"></script><script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments)}gtag('js',new Date());gtag('config','${config.analytics.publicId}');</script>`:'';
for(const route of routes.filter(r=>r.published)){
 ...
 let content;
 if(route.kind==='home')content=normalize(read('templates/home.html')).replace('<!--home-hub-->',homeHub()).replace('<!--lead-form-->',leadForm({routes,config,wa,source:'/',prefix:'c',heading:'Contanos qué celebrás',hint:'Dos pasos cortos. Gratis y sin compromiso.'}));
