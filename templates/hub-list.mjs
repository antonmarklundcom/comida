...
import {minutes,faqHtml,sectionHtml} from './recipe-lib.mjs';
...
export function hubList(kind){return ({page:h,route,routes,pages})=>{
  ...
  const card=i=>{const r=routes.find(r=>r.id===i.id);return `<a class="rcard tone-${tone(i.id)}" href="${attr(r.path)}" data-cats="${attr((i.category||[]).join(' '))}" data-name="${attr((i.label+' '+(i.keywords||[]).join(' ')).toLowerCase())}"><span class="rcard-kind">${esc(i.course||(kind==='cut'?i.animal||'Corte':'Guía'))}</span><strong>${esc(i.label)}</strong>${i.cardText?`<span class="rcard-text">${esc(i.cardText)}</span>`:''}${i.times?`<span class="rcard-meta">${esc(minutes((i.times.prep||0)+(i.times.cook||0)+(i.times.rest||0)))} · ${esc(i.difficulty)}</span>`:''}</a>`};
  ...
  return `<main id="top" class="content-page hub-page"><div class="wrap">${breadcrumbs(route,routes)}<h1>${esc(h.h1)}</h1>${h.intro.map(p=>`<p class="lead">${esc(p)}</p>`).join('')}${filters}<div class="rcards rcards-hub">${items.map(card).join('')}</div>${(h.sections||[]).map(sectionHtml).join('')}${faqHtml(h.faq,'Preguntas frecuentes')}</div></main>`;
}}
